-- Patch: Grade picks strictly via leagues.locked_lines (no scoreboard lines)
-- Run this in Supabase SQL editor or via `npx supabase db remote execute --stdin`
...
BEGIN
  ...
  FOR v_user_id IN SELECT UNNEST(v_league.members) AS user_id LOOP
    v_total_points := 0;
    v_winner_correct := 0;
    ...
    LOOP
      ...
      -- Spread grading using league.locked_lines only
      ...
      -- Total grading using league.locked_lines only
